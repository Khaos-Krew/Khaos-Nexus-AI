...
export class SupabaseAuthVerifier {
  ...
  async verify(accessToken) {
    ...
    if (!response.ok) {
      throw serviceError(describeSupabaseFailure(payload, "Invalid or expired access token"), 401);
    }
    ...
    return {
      ...
      appMetadata: payload.app_metadata && typeof payload.app_metadata === "object"
...
export class SupabaseCampaignStore {
  ...
  async get(id, context) {
    const workspace = await this.getWorkspace(id, context);
    return workspaceCampaignToService(workspace);
  }

  async create(campaign, context) {
    requireContext(context);
    assertUuid(campaign.tenantId, "tenantId");
    const workspace = await this.client.rpc(
  ...
  async update(campaign, context) {
    requireContext(context);
    assertUuid(campaign.id, "campaign id");
    const workspace = await this.client.rpc(
  ...
  async approveHomebrew(campaignId, homebrewId, context) {
    requireContext(context);
    assertUuid(campaignId, "campaign id");
    assertUuid(homebrewId, "homebrew id");
    return this.client.rpc(
